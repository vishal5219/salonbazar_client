...
import {
  FiGrid, FiUsers, FiCalendar, FiBarChart2, FiUserPlus, FiSettings, FiExternalLink, FiCheckCircle,
} from 'react-icons/fi'
import { ROLES } from '@/constants/roles'
import { roleLabel } from '@/constants/roles'
import { DASHBOARD_NAV, DASHBOARD_PATHS } from '@/constants/dashboardRoutes'
import styles from './DashSidebar.module.css'

const NAV_ICONS = {
  overview: FiGrid,
  queue: FiUsers,
  queueCompleted: FiCheckCircle,
...
export default function DashSidebar({ salon, role, onNavigate }) {
  const { queue, bookings, queueStats } = useSelector(s => s.dashboard)
  ...
  const getBadge = (badgeKey) => {
    ...
    if (badgeKey === 'completed')
      return queueStats?.completedToday || null
